...
*   **Slide 4: Sơ đồ kiến trúc Pipeline hệ thống** – Vẽ luồng dữ liệu rõ ràng: Tệp thô CSV $\rightarrow$ Spark Cluster (Tính toán ẩn) $\rightarrow$ Đóng gói sang MongoDB Database $\rightarrow$ Giao diện Web (Tương tác trực diện với người dùng).
...
*   **Slide 8: TRỌNG TÂM DEMO SẢN PHẨM** – Chiếu màn hình giao diện Web khi nhập User ID thật, kết quả danh sách các tên phim hiển thị mượt mà ngay lập tức. Giải thích rõ: Web chạy nhanh là nhờ lấy kết quả sẵn từ MongoDB chứ không bắt Spark tính lại từ đầu.
...
> "Nhóm em không chạy mô hình trên máy đơn lẻ với dữ liệu giả. Hệ thống được thiết kế theo chuẩn Big Data: Tách biệt phần Tính toán phân tán (Spark) và phần Truy vấn tốc độ cao (MongoDB). Nhờ vậy, hệ thống vừa có thể xử lý lượng dữ liệu khổng lồ, vừa đảm bảo trải nghiệm người dùng trên Web cực kỳ mượt mà."
...
2.  **Phần lưu trữ và hiển thị (MongoDB & Web - Chạy Online):** Kết quả của Spark được đóng gói vào MongoDB. Khi nhập User ID trên Web, hệ thống chỉ việc "nhấc" kết quả từ MongoDB ra, không bắt Spark tính lại, giúp tốc độ phản hồi cực kỳ nhanh chóng.
