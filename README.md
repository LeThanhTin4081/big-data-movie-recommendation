...
## 🚀 Kiến trúc Hệ thống
1. **Luồng tính toán ẩnn (Offline):** Dữ liệu thô $\rightarrow$ `src/etl` (Làm sạch) $\rightarrow$ `src/ml` (Chạy AI/ALS).
2. **Luồng phụccc vụ (Online):** Kết quả AI $\rightarrow$ `src/nosql` (Lưu trữ) $\rightarrow$ `src/web` (Giao diện siêu tốc).
